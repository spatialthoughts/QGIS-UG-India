---
type: "page"
title: "Blog"
subtitle: "Tips, resources, and articles for QGIS users"
draft: false
heroSize: "is-medium"
HeroImage: "/img/hegobg1.webp"
HasBanner: true
sidebar: true
---

{{< content-start >}}

## Weekly QGIS Tips

Quick, practical tips to improve your QGIS workflow in India. New tips every week.

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-full">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

- **Sketcher Tool** - Quick annotations without creating layers. Go to View > Sketcher to add markup during presentations or reviews.

*More tips coming soon!*

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

---

## QGIS Resources

Curated collection of styles, palettes, projects, models, and more.

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-half">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

### Styles
Symbol libraries and styling presets for beautiful maps.

- [QGIS Style Hub](https://plugins.qgis.org/styles/)
- [SLYR](https://north-road.com/slyr/) - Convert ArcGIS styles

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}

{{< column-start class="is-flex-direction-column is-half">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

### Palettes
Color schemes for cartography.

- [ColorBrewer](https://colorbrewer2.org/)
- [CARTOColors](https://carto.com/carto-colors/)

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-half">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

### Projects
Sample QGIS project files and templates.

*Coming soon!*

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}

{{< column-start class="is-flex-direction-column is-half">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

### Models
Processing models for automated workflows.

*Coming soon!*

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-half">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

### PyQGIS
Python scripts and tutorials.

- [PyQGIS Cookbook](https://docs.qgis.org/latest/en/docs/pyqgis_developer_cookbook/)

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}

{{< column-start class="is-flex-direction-column is-half">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

### Actions
Layer actions and interactive tools.

*Coming soon!*

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-half">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

### Plugins
Recommended plugins.

- [QGIS Plugins Repository](https://plugins.qgis.org/)

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

---

## Articles

In-depth tutorials and guides.

{{< columns-start >}}
{{< column-start class="is-flex-direction-column is-full">}}
{{< rich-box-start >}}
{{< rich-content-start themeClass="coloring-1" >}}

*Coming soon!*

{{< rich-content-end >}}
{{< rich-box-end >}}
{{< column-end >}}
{{< columns-end >}}

{{< content-end >}}
